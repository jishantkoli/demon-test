...
export const getStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Parallelize independent database counts to reduce total response time
    // Bolt Optimization: Replaces 13+ sequential calls with a single Promise.all
    const [
      totalUsers,
      activeForms,
      draftForms,
    ...
    if (role === 'functionary' && functionaryStats) {
      const [totalNom, pendingNom, invitedNom, completedNom] = functionaryStats;
      totalNominations = totalNom;
      nominationsByStatus = {
        pending: pendingNom,
        invited: invitedNom,
        completed: completedNom,
      };

...
export const getStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Optimized stats fetching using Promise.all and Aggregation Pipelines
    // This reduces the number of database roundtrips from ~15 to 4-5
    ...
    // Parse results from aggregation
    ...
    const activeForms = formStats.find((f: any) => f._id === 'active')?.count || 0;
    const draftForms = formStats.find((f: any) => f._id === 'draft')?.count || 0;
    const expiredForms = formStats.find((f: any) => f._id === 'expired')?.count || 0;
    ...
    if (role === 'functionary' && nominationStats.length > 0) {
      ...
      nominationsByStatus = {
        pending: nominationStats[0]?.byStatus.find((s: any) => s._id === 'pending')?.count || 0,
        invited: nominationStats[0]?.byStatus.find((s: any) => s._id === 'invited')?.count || 0,
        completed: nominationStats[0]?.byStatus.find((s: any) => s._id === 'completed')?.count || 0,
      };

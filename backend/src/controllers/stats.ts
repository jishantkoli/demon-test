...
export const getStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Parallelize all count queries to improve performance
    // Optimization: Reduces database roundtrips by executing queries concurrently
    const [
      totalUsers,
      activeForms,
      draftForms,
    ...
    if (role === 'functionary') {
      const [totalNom, nomPending, nomInvited, nomCompleted] = await Promise.all([
